...
func init() {
	...
	privKey.D, _ = new(big.Int).SetString(D, 10)
	privKey.N, _ = new(big.Int).SetString(N, 16)
}
...
/*
func (m *RSACryptor) doRSAdecrypt(b []byte) []byte {
	c := new(big.Int)
...
func DoRSAdecrypt(b []byte) []byte {
	...
	c := new(big.Int)
	c.Exp(new(big.Int).SetBytes(z), privKey.D, privKey.N)
...
func DoAES256IGEdecrypt(data, key, iv []byte) ([]byte, error) {
	block, err := aes.NewCipher(key)
	if err != nil {
		print("error ige.decrypt\n")
		return nil, err
	}
	if len(data) < aes.BlockSize {
		return nil, errors.New("AES256IGE: Data too small to decrypt")
	}
	if len(data)%aes.BlockSize != 0 { // data harus kelipatan 16
		return nil, errors.New("AES256IGE: Data not divisible by block Size")
	}
