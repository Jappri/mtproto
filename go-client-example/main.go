...
import (
	...
	"../../server/go-server-poc/library/mtproto"
	// "github.com/graflib/mtproto"
)
...
func (cli *TelegramCLI) LoadContacts() error {
	tl, err := cli.mtproto.ContactsGetContacts(0)
	if err != nil {
		return err
	}
...
func main() {
	...
	// NewMTProto(newSession bool, serverAddr string, useIPv6 bool, authkeyfile string, appConfig Configuration) (*MTProto, error) {

	newconf, err := mtproto.NewConfiguration(147180, "5c79b8f7abc7eb3f004537c1ecf3de10", "0.7", "deviceModel", "0.7.1", "en")
	// LoadContacts
	// mtproto, err := mtproto.NewMTProto(147180, "5c79b8f7abc7eb3f004537c1ecf3de10", mtproto.WithAuthFile("./telegramgo", false))\
	// ip := "91.108.56.149:443"
	ip := "127.0.0.1:8800"
	mtproto, err := mtproto.NewMTProto(false, ip, false, "authkeyfile.txt", *newconf)
	// telegramgo file diisi dengan data-data yang diperlukan untuk connect ke server. Diisi ketika pertama kali run, dengan catatan: nggak ada error yang terjadi sampai proses penulisan
	if err != nil {
		log.Fatal(err)
	}
